...
-- numGreaterThanTen ys = length (filter (\(a, b) -> b > 10) ys)
numGreaterThanTen = length.filter (\(a, b) -> b > 10)
...
mapTree f = foldTree Leaf (\r x l -> Node l (f x) r)
...
-- pair cmpA cmpB (a, b) (c, d) = if(cmpA a c == EQ) then cmpB b d else cmpA a c
pair cmpA cmpB = \(a, b) (c, d) -> if (cmpA a c == EQ) then cmpB b d else cmpA a c
...
sortOnReverseSndLength xs = mergeSortWith (invert (compare `on` (length.snd))) xs
...
rectangle w h = \(x, y) -> (x <= w / 2) && (y <= h / 2) && (x <= (w / 2 * (-1))) && (y <= (h / 2 * (-1)))
...
pictureAND p1 p2 = \(x, y) -> p1(x, y) && p2(x, y)

pictureOR :: Picture Bool -> Picture Bool -> Picture Bool
pictureOR p1 p2 = \(x, y) -> p1(x, y) || p2(x, y)

pictureNOT :: Picture Bool -> Picture Bool
pictureNOT p = \(x, y) -> (p(x, y) == False)
...
--true if smaller than big circle, but bigger than small one
...
colourIn pBool colTrue colFalse = \(x, y) -> if(pBool(x, y)) then colTrue else colFalse
...
mapPicture f picA = \(x, y) -> f (picA (x, y))
...
mapPicture2 f picA picB = \(x, y) -> f (picA (x, y)) (picB (x, y)) 
...
pictureApply picFun picA = \(x, y) -> picFun (x, y) (picA (x, y))
...
mapPicture2' :: (a -> b -> c) -> Picture a -> Picture b -> Picture c
mapPicture2' f pic1 pic2 = everywhere f `pictureApply` pic1 `pictureApply` pic2
...
cut picBool pic = mapPicture2 (\x y -> if (x) then y else mempty) picBool pic 
...
-- x the same, y opposite
flipTopBottom :: Picture a -> Picture a
flipTopBottom pic = transform (\(x, y) -> (x, -y)) pic

flipLeftRight :: Picture a -> Picture a
flipLeftRight pic = transform (\(x, y) -> (-x, y)) pic
...
bprocessToProcess (BOutput b procces) = Output b (bprocessToProcess procces)
bprocessToProcess  (BInput p1 p2) = Input (\b -> if b then (bprocessToProcess p1) else (bprocessToProcess p2)) 

processToBProcess :: Process Bool () -> BProcess
processToBProcess (End()) = BEnd
processToBProcess  (Input v) = (BInput (processToBProcess (v True)) (processToBProcess (v False)))
processToBProcess (Output b proccess)   = (BOutput b (processToBProcess proccess))
...
sequ (Output v k) func = (Output v (sequ k func))
...
echoFromSequ = sequ input(\x -> output x)
...
chatBot = Output "Hello! What is your name?" (Input (\v -> Output ("Hello " ++ v) (End())))
...
eqProcess (End ())       (End ()) = True  -- are these processes equal?
eqProcess (Output b1 k1) (Output b2 k2) = if(b1 == b2) then eqProcess k1 k2 else False
  -- do they output the same thing, and do they
  -- do the same thing after that?
-- eqProcess (Input k1)     (Input k2) = eqProcess (Input (\x -> if(x== True) then End() else End())) (Input k2)
eqProcess (Input k1)     (Input k2) = eqProcess (k1 True) (k2 True) && eqProcess (k1 False) (k2 False)
