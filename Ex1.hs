...
consAll :: a -> [[a]] -> [[a]]
consAll a ([])       = []
consAll a ((x) : xs) = (a:x) : consAll a xs
...
splitOnHelper x [] [] = [[]]
splitOnHelper x ys [] = [reverse ys]
splitOnHelper x (ys) (z:zs)
    | x /= z          = splitOnHelper x (z : ys) zs
    | otherwise       = [reverse ys] ++ splitOnHelper x [] (zs)
...
layOutLines w 0 line (word:words)      = layOutLines w (plusWord (-1) word) (word:line) words
layOutLines w col line (word:words)
   | plusWord col word <= w            = layOutLines w (plusWord col word) (word:line) words 
   | otherwise                         = [reverse line] ++ layOutLines w 0 [] (word:words) 
...
atEnd :: Cursor -> Bool
atEnd (AtEnd before)                  = True 
atEnd (Within beggining cursor after) = False 


atStart :: Cursor -> Bool
atStart (AtEnd before)               = False
atStart (Within [] cursor after)     = True
atStart (Within before cursor after) = False
...
moveLeft (AtEnd [])                    = AtEnd []
moveLeft (AtEnd (b:before))            = Within before b []
moveLeft (Within [] cur after)         = Within [] cur after
--moveLeft (Within (b:before) cur [])    = Within before b [cur]
moveLeft (Within (b:before) cur after) = Within before b (cur:after)
...
insert x (AtEnd before)               = AtEnd (x:before)
insert x (Within before cursor after) = Within (x:before) cursor after 
...
backspace (AtEnd (last:after))     = AtEnd after
backspace (Within [] cursor after) = Within [] cursor after
backspace (Within (last : before) cursor after) = Within before cursor after
...
delete (AtEnd before)                   =  AtEnd before
delete (Within before cursor [])        =   AtEnd before
delete (Within before cursor (a:after)) =   Within before a after 
...
overwrite a (AtEnd before)               = Within before a ""
overwrite a (Within before cursor after) = Within before a after 
...
currentLine (LineCursor above cursor below) = cursor

updateLine :: LineCursor -> Cursor -> LineCursor
updateLine (LineCursor above cursor below) newCursor = LineCursor above newCursor below

moveUp :: LineCursor -> LineCursor
moveUp (LineCursor [] cursor below)        = LineCursor [] cursor below
moveUp (LineCursor (a:above) cursor below) = LineCursor above (toCursor a) ((fromCursor cursor) : below)

moveDown :: LineCursor -> LineCursor
moveDown (LineCursor above cursor [])        = LineCursor above cursor []
moveDown (LineCursor above cursor (b:below)) = LineCursor ((fromCursor cursor) : above) (toCursor b) below
...
insertKey k v [] = [(k,v)]
insertKey k v ((k',v') : kv)
   | k == k'     = ((k,v) : kv)
   | k < k'      = (k,v) : ((k',v') : kv)
   | otherwise   = (k',v') : insertKey k v kv
...
{- 1.3.5 getKey
   ...
   Examples:
      ...
      getKey ["a","b"] (Store [("a",Store [("b",Value "X")])]) == Just (Value "X")
...
getKey k (Value s)       = Nothing
getKey [] v              = Nothing
getKey k (Store [])      = Nothing
getKey [k] (Store ((k',x) : ys))
    | k == k'            = Just x
    | otherwise          = getKey [k] (Store ys)
getKey (k:kx) (Store ((k',x) : ys))
   | k == k'             = getKey kx x
   | otherwise           = getKey (k:kx) (Store ys)
...
getValue [] v              = KeyMissing
getValue k (Value s)       = KeyMissing
getValue k (Store [])      = KeyMissing
getValue [k] (Store ((y, Value x) : ys))
    | k == y               = Ok x
    | otherwise            = getValue [k] (Store ys)
getValue [k] (Store ((y, v) : ys)) 
    | k == y               = KeyNotAValue
    | otherwise            = getValue [k] (Store ys)
getValue (k:kx) (Store ((k',x):ys))
   | k == k'               = getValue kx x
   | otherwise             = getValue (k:kx) (Store ys)
...
merge (Store x) (Value y)                          = Store x
merge (Value y) (Store x)                          = Store x
merge (Store []) (Store [])                        = emptyConfig
merge c (Store [])                                 = c
merge (Store []) c                                 = c
merge (Value v) (Value v')                         = Value v'
merge (Store ((k,v) : ys)) (Store ((k',v') : ys')) = Store (mergeKVs ((k,v) : ys) ((k',v') : ys'))
...
mergeKVs (ys) []  = ys
mergeKVs [] (ys)  = ys
mergeKVs ((s, c) : ys) ((s', c') : ys')
   | s == s'      = (s, merge (c) (c')) : mergeKVs ys ys'
 --  | s == s'      = (s, c') : mergeKVs ys ys'
   | s < s'       = (s, c) : mergeKVs ys ((s', c') : ys')
   | otherwise    = (s', c') : mergeKVs ((s, c) : ys) ys'    
...
update k [] c              = c
update [] s (Store x)      = Value s
update [] s (Value x)      = Value s
update k s (Value x)       = Value x
update [y] s (Store [])    = Store[(y, (Value s))]
update (y:ys) s (Store []) = Store [(y, (update ys s (Store [])))]
update (y:ys) s (Store((x, c) : xs))
   | y == x                = Store [(y, (update ys s c))]
   | y > x                 = merge (Store [(x,c)]) (update (y:ys) s (Store xs))
   | otherwise             = merge (update (y:ys) s (Store [])) (Store((x, c) : xs))
...
output :: Bool -> Process
output True  = Output True End
output False = Output False End
...
outputs []     = End
outputs (b:bs) = Output b (outputs bs)
...
andGate :: Process
andGate = Input (Input (Output True End) (Output False End)) (Input (Output False End) (Output True End))
...
expects [b]
  | b == True     = Input (Output True End) (Output False End)
  | b == False    = Input (Output False End) (Output True End)
expects (b:bs)
  | b == True     = Input (expects bs) (helperExpects bs)
  | otherwise     = Input (helperExpects bs) (expects bs)
 
helperExpects :: [Bool] -> Process
helperExpects []     = Output False End
helperExpects [b]    = Input (Output False End) (Output False End)
helperExpects (b:bs) = Input (helperExpects bs) (helperExpects bs)
...
sequ End             p2 = p2
sequ (Output b p1)   p2 = Output b (sequ p1 p2)
...
pipe p1            End           = End
pipe p1            (Output b p2) = Output b (pipe p1 p2)  -- what happens next?
pipe End           (Input t f)   = End
  -- the second process is hungry, but it starves to death!
pipe (Output b p)  (Input t f)   
   | b == True = pipe p t
   | otherwise = pipe p f
  -- what goes here?
...
--Input (Output True (Input (Output False End) (Output True End))) (Output False (Input (Output False End) (Output True End)))
